...
public class MainScript : MonoBehaviour {
        ...
        {
			AdsManager.Instance.TopAdeptiveBanner();
        }
	...
	public void refreshView(){
		...
		public void gameWin(bool directWin = false){
				...
				GameManager.getInstance ().playSfx ("youwin");
				StartCoroutine ("win");
		...
		IEnumerator win(){
				...
				if (GameData.getInstance().cLevel < GameData.totalLevel) {
						panelWin.transform.Find("panel").transform.Find ("btnEnd").gameObject.SetActive (false);
				} else {
		...
		//}
		//if (GameData.instance.cLevel > 3)
		//{
		//	int trnd = Random.Range(0, 5);
		//	if (trnd == 1)
		//	{
		...
		panelWin.SetActive(true);
				...
				panelWin_.transform.Find ("winTitle").transform.Find("Text 1").GetComponent<Text> ().text = Localization.Instance.GetString ("wintitle");
				panelWin_.transform.Find ("winTitle").transform.Find("Text 2").GetComponent<Text>().text = Localization.Instance.GetString("wintitle");
		...
		//		lbScore.Text = LanguageManager.Instance.GetTextValue("GAME_SCORE") +": " + tScore;
				...
				panelWin_.transform.Find("Panel").transform.Find("levelscore").GetComponent<Text>().text = tScore.ToString() ;
		int tminute = Mathf.FloorToInt((float)timeCount / 60f);
		int tSecond = timeCount - tminute*60;
		
		panelWin_.transform.Find("Panel").transform.Find("txtTime").GetComponent<Text>().text = string.Format("{0:D2}:{1:D2}", tminute, tSecond);
		...
		if (GameData.instance.cLevel >= 3)
		{
			PlayerPrefs.SetInt("rateAppear", 1);
		}
		int cRate = PlayerPrefs.GetInt("rateClose");
			if (GameData.instance.cLevel >= 3 && cRate < 2)
			{
		...
		private void PopupClosedMethod()
		{
			...
			int cRate = PlayerPrefs.GetInt("rateClose");
			cRate++;
			PlayerPrefs.SetInt("rateClose", cRate);
		}
...
//				panelFail_.transform.Find ("btnMenu").GetComponentInChildren<Text> ().text = Localization.Instance.GetString ("btnMenu");
